...
function TradeConfirmationPage() {
  ...
  useEffect(() => {
    ...
    async function loadTrade() {
      ...
      try {
        const data = await getFarmerTrade(flowIds.tradeId)
        if (!isActive) {
          return
        }
        ...
        setScreenState({
          status: "error",
          data: null,
          error: error.message || "Unable to load the trade confirmation.",
        })
      }
    }
  ...
  if (!flowIds.tradeId) {
    return <Navigate replace to={ROUTES.FARMER_AI_TRADE_PROPOSAL} />
  }

  const trade = screenState.data
      ...
      >
        <main className="min-h-screen max-w-md mx-auto flex flex-col px-6 pt-10 pb-8">
          ...
          {screenState.status === "loading" && (
            <div className="mb-6 rounded-[2rem] border border-outline-variant/30 bg-surface-container-lowest p-6 shadow-sm">
              <p className="text-sm font-semibold text-on-surface-variant">Loading transaction details and planting guidance...</p>
            </div>
          )}

          {screenState.status === "error" && (
            <div className="mb-6 rounded-[2rem] border border-error/20 bg-error-container/60 p-6 shadow-sm">
            ...
            <>
                ...
                <div className="relative z-10 flex items-start gap-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <p className="text-sm text-on-surface leading-snug">
                      These new inputs are projected to increase your <span className="text-primary font-bold">yield by {trade.projected_yield_uplift_pct}%</span>. We've started drafting your harvest listing.
                    </p>
              ...
              <section className="bg-surface-container-low/40 border border-outline-variant/30 rounded-3xl p-6 space-y-6 mb-8">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-on-surface-variant font-mono text-[11px] font-bold bg-surface-variant/50 px-2 py-0.5 rounded">{trade.transaction_code}</span>
                </div>
                <div className="space-y-5">
                  <div className="bg-surface-container-lowest p-4 rounded-full flex items-center gap-4 border border-outline-variant/10 shadow-sm">
                    ...
                    <div>
                      ...
                      <p className="font-headline font-bold text-on-surface text-lg">{trade.counterparty_name}</p>
                    </div>
                  </div>
                  <div className="grid grid-cols-2 gap-4 px-2">
                    <div className="space-y-1">
                      ...
                      <div className="flex flex-col">
                        <span className="text-xs font-medium text-on-surface-variant line-through opacity-50">{formatQuantity(trade.offer_quantity, trade.offer_unit)} {trade.offer_item_name}</span>
                        <span className="text-sm font-bold text-primary">{formatQuantity(trade.requested_quantity, trade.requested_unit)} {trade.requested_item_name}</span>
                      </div>
                    </div>
                    <div className="space-y-1 border-l border-outline-variant/30 pl-4">
                      ...
                      <div className="flex flex-col">
                        <span className="text-xs font-medium text-on-surface-variant">{formatDateTime(trade.meeting_at)}</span>
                        <span className="text-sm font-bold text-on-surface">{trade.meeting_point_name}</span>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div className="space-y-12">
                  ...
                  <div className="flex items-start gap-6 relative step-line">
                    ...
                    <div className="pt-1.5 flex-1">
                      ...
                      <p className="text-xs text-on-surface-variant leading-relaxed">{trade.planting_prompt}</p>
                    </div>
                  </div>
